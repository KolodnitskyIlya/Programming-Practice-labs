...
class World:
    ...
    def step(self):
        ...
        for entity in self.entities[:]:
            entity.update()
        print(f"Time: {self.time_of_day}, Entities: {len(self.entities)}")
...
def simulate(world_size=10, steps=20, plant_density=0.2, animal_density=0.05):
...
class TestEcosystem(unittest.TestCase):
    def test_registry(self):
        self.assertIn(Lumiere, EcosystemMeta.registry['plants'])
        self.assertIn(Pauvre, EcosystemMeta.registry['animals'])

    def test_methods(self):
        p = Pauvre()
        self.assertTrue(hasattr(p, 'eat'))
        self.assertTrue(hasattr(p, 'move'))
        self.assertTrue(hasattr(p, 'reproduce'))

        l = Lumiere()
        self.assertTrue(hasattr(l, 'spread'))

    def test_spread_behavior(self):
        w = World(5,5)
        l = Lumiere()
        w.add_entity(l, Position(2,2))
        l.active = True
        initial_count = len(w.entities)
        l.spread()
        self.assertGreaterEqual(len(w.entities), initial_count)
